...
public class ResNet50TaskDispatcher implements ITaskDispatcher {
	
	private final static Logger log = LogManager.getLogger (ResNet50TaskDispatcher.class);
	...
	public ResNet50TaskDispatcher (Dataflow df) {
		...
		if (ModelConf.getInstance ().getDataset (phase).getType () != DatasetType.RECORD)
