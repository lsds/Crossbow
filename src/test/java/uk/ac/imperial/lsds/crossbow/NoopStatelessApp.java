...
import uk.ac.imperial.lsds.crossbow.kernel.NoopStateless;
...
public class NoopStatelessApp {
	
	@SuppressWarnings("unused")
	private final static Logger log = LogManager.getLogger (NoopStatelessApp.class);
	
	public static void main (String [] args) throws Exception {
		
		long startTime, dt;
		
		Options options = new Options (NoopStatelessApp.class.getName());
		
		/* Generic options */
		options.addOption ("--N",             "Train for N units", true,  Integer.class, "10000000");
		options.addOption ("--training-unit", "Training unit",     true,   String.class, "tasks");
		
		/* Application-specific options */
		options.addOption ("--chain",         "Dataflow depth", true, Integer.class, "2");
		...
		SystemConf.getInstance ()
			...
			.setNumberOfGPUCallbackHandlers (1)
		...
		ModelConf.getInstance ().setBatchSize (64).setWpc (1).setSlack (0);
		...
		Operator op1 = new Operator ("Noop-1", new NoopStateless (new NoopConf()));
		...
		if (depth > 1) {
			...
			for (int i = 1; i < depth; i++) {
				node = node.connectTo(new DataflowNode (new Operator (String.format("Noop-%d", (i + 1)), new NoopStateless (new NoopConf()))));
			}
		}
		...
		/* Dump configurations on screen */
		SystemConf.getInstance ().dump (); ModelConf.getInstance ().dump ();
