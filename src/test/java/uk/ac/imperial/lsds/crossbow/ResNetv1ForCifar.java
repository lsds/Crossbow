...
public class ResNetv1ForCifar {
	...
	// private static double EPSILON = 0.00001D;
	// private static double ALPHA   = 0.999D;
	...
	public static DataflowNode [] buildResNetShortcut (DataflowNode [] input, String prefix, boolean match, int numberofoutputs, int stride, DataflowNode [] gradient) {
		...
		if (! match) {
			...
			convConf.setWeightInitialiser (new InitialiserConf ().setType (initialiserType));
	...
	public static DataflowNode [] buildResNetBasicUnit (int n, int stride, DataflowNode [] input, String prefix, DataflowNode [] gradient) {
		...
		convConf [0].setWeightInitialiser (new InitialiserConf ().setType (initialiserType));
		...
		convConf [1].setWeightInitialiser (new InitialiserConf ().setType (initialiserType));
	...
	public static DataflowNode [] buildResNetBottleneckedUnit (int n, int stride, DataflowNode [] input, String prefix, DataflowNode [] gradient) {
		...
		convConf [0].setStrideSize (2).setStrideHeight (1).setStrideWidth (1);
		...
		convConf [1].setKernelSize  (2).setKernelHeight  (  3   ).setKernelWidth  (  3   );
		convConf [1].setStrideSize  (2).setStrideHeight  (stride).setStrideWidth  (stride);
        convConf [1].setPaddingSize (2).setPaddingHeight (  1   ).setPaddingWidth (  1   );
		...
		convConf [2].setWeightInitialiser (new InitialiserConf ().setType (initialiserType));
	...
	public static SubGraph [] buildResNetImageNet (int [] blocks, int [] features, SolverConf solverConf, boolean bottleneck, int numclasses, boolean transform) {
		...
		innerproductConf.setWeightInitialiser (new InitialiserConf ().setType (initialiserType));
	...
	public static SubGraph [] buildResNetCifar10 (int blocks, int [] features, SolverConf solverConf) {
		...
		innerproductConf.setWeightInitialiser (new InitialiserConf ().setType (initialiserType));
		// innerproductConf.setWeightInitialiser (new InitialiserConf ().setType (InitialiserType.GAUSSIAN).truncate(true).setStd(0.125F));
		innerproductConf.setBiasInitialiser   (new InitialiserConf ().setType (InitialiserType.CONSTANT));
	...
	public static void main (String [] args) throws Exception {
		...
		options.addOption ("--dataset-name",   	"Dataset name",           true,   String.class, "cifar-10");
		options.addOption ("--layers",   		"Number of layers",       true,  Integer.class,       "32");
		options.addOption ("--data-directory",  "Data directory",         true,   String.class, "/data/crossbow/cifar-10/padded/b-064");
		...
		ModelConf.getInstance ().setBatchSize (64).setWpc (wpc).setSlack (0).setUpdateModel(UpdateModel.WORKER);
		...
		if (datasetName.equals("imagenet")) {
			...
			String dataDirectory = String.format("/data/crossbow/imagenet/ilsvrc2012/helper/b-%03d/", batch_size);
